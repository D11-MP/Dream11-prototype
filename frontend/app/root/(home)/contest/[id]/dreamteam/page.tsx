...
export default function Page() {
  ...
  let filteredFinal = final.filter(
    (player: Data) =>
      !outPlayers.some((outPlayer) => outPlayer.name === player.name)
  );
  filteredFinal = filteredFinal.filter(
    (player: Data) =>
      !inPlayers.some((outPlayer) => outPlayer.name === player.name)
  );
  let sortedFinal = filteredFinal.sort(
    (a: any, b: any) => b.predicted_points - a.predicted_points
  );
    ...
    <div className="flex flex-col w-full text-center pb-10">
      ...
      <div className="flex justify-center min-h-fit w-[80vw] mx-0 mt-8 gap-4">
        <div className="w-[60%]">
          <DreamTeamMatchCard
            match={match[0]}
            total_predicted_points={totalPredictedPoints}
          />
